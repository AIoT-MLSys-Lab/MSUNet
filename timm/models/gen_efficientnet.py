...
from .conv2d_helpers import select_conv2d, conv2d_pad, MixedAvgPooling, _get_padding
from timm.data import IMAGENET_DEFAULT_MEAN, IMAGENET_DEFAULT_STD, CIFAR100_DEFAULT_MEAN, CIFAR100_DEFAULT_STD
...
default_cfgs = {
    ...
    'MSUnet_CIFAR100': _cfg(url='',input_size=(3, 32, 32), mean=CIFAR100_DEFAULT_MEAN, std=CIFAR100_DEFAULT_STD, pool_size=(8, 8), num_classes=100),
}
    ...
    for op in ops:
        ...
        if op in ['seConvFc','seLSTM']:
            ...
            if v == 're':
                ...
                continue
            options[key] = value
        elif op.startswith('shuf'):
            shuffle_type = op.split('.')[1]
        else:
            # all numeric options
...
def _gen_MSUnet_CIFAR100(channel_multiplier=1.0, depth_multiplier=1.0, num_classes=100, **kwargs):
    """Creates a MixNet Medium-Large model.
    ...
    arch_def = [
        ...
        ['ir_r1_k3.5.7_s2_e5_c128_se0.5_nsw', 'ir_r1_k3.5_a1.1_p1.1_s1_e6_c128_se0.5_nsw'],
        ['ir_r1_k3.5.7_p1.1_s2_e5_c256_se0.25_nsw', 'ir_r1_k3.5_p1.1_s1_e4_c256_se0.25_nsw'],
    ]
...
class SqueezeExcite(nn.Module):
    def __init__(self, in_chs, reduce_chs=None, act_fn=F.relu, gate_fn=F.sigmoid):
        super(SqueezeExcite, self).__init__()
        self.act_fn = get_act_module(act_fn)
        self.gate_fn = get_act_module(gate_fn)
...
class DepthwiseSeparableConv(nn.Module):
    ...
    def __init__(self, in_chs, out_chs, dw_kernel_size=3,
        ...
        self.act_fn1 = get_act_module(act_fn)
        self.act_fn2 = get_act_module(act_fn)
        self.drop_connect_rate = drop_connect_rate
...
class InvertedResidual(nn.Module):
    ...
    def __init__(self, in_chs, out_chs, dw_kernel_size=3,
                 ...
                 se_ratio=0., se_reduce_mid=False, se_gate_fn=sigmoid,
                 shuffle_type=None, bn_args=_BN_ARGS_PT, drop_connect_rate=0., avgConcat=None, se_type=None):
        ...
        self.avgConcat = None
        if avgConcat: # FixMe: the split is consistant with the 1x1 conv not dw conv
            assert not self.has_residual, 'Using avgConcat does not allow residual skip connection'
            self.avgConcat = MixedAvgPooling(self.conv_pw.splits, self.conv_dw.kernel_size, self.conv_dw.stride)

        # Squeeze-and-excitation
        if self.has_se:
            se_base_chs = mid_chs if se_reduce_mid else in_chs
            if se_type is not None:
                raise Exception('The SE type {} is not supported'.format(se_type))
            else:
                self.se = SqueezeExcite(
                    mid_chs, reduce_chs=max(1, int(se_base_chs * se_ratio)), act_fn=act_fn, gate_fn=se_gate_fn)
    ...
    def forward(self, x):
        ...
        if self.has_residual:
            if self.drop_connect_rate > 0.:
                x = drop_connect(x, self.training, self.drop_connect_rate)
            x += residual
        elif self.avgConcat is not None:
            residual = self.avgConcat(residual)
...
class GenEfficientNet(nn.Module):
    ...
    def __init__(self, block_args, num_classes=1000, in_chans=3, stem_size=32, num_features=1280,
                 ...
                 global_pool='avg', head_conv='default', weight_init='goog', stem_stride=None):
        ...
        if stem_stride is not None:
            self.conv_stem = select_conv2d(in_chans, stem_size, 3, stride=stem_stride, padding=pad_type)
        else:
...
@register_model
def MSUnet_CIFAR100(pretrained=False, num_classes=1000, in_chans=3, **kwargs):
    """Creates a MixNet Medium model.
    """
    default_cfg = default_cfgs['MSUnet_CIFAR100']
    model = _gen_MSUnet_CIFAR100(
        channel_multiplier=1.0, num_classes=num_classes, in_chans=in_chans, **kwargs)
    model.default_cfg = default_cfg
    if pretrained:
        raise NotImplementedError('MSUnet_CIFAR100 does not have pretrained model')
        #load_pretrained(model, default_cfg, num_classes, in_chans)
    return model
...
def get_act_module(act_fn):
    if act_fn == F.relu or isinstance(act_fn, nn.ReLU):
        return ReLU_Module()
    elif act_fn == swish:
        return Swish_Module()
    elif act_fn in [sigmoid, F.sigmoid] or isinstance(act_fn, nn.Sigmoid):
        return Sigmoid_Module()
    else:
        raise NotImplementedError('The activation function is not implemented.')
