...
def _split_channels(num_chan, num_groups, uneven_split=False):
    ...
    if isinstance(padding, str):
        ...
        if padding == 'same':
            ...
            if _is_static_pad(kernel_size, **kwargs):
                ...
                raise NotImplementedError('Dynamic padding is not implemented by Yu.')
        elif padding == 'valid':
...
class MixedConv2d(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, kernel_size=3,
        ...
        if depthwise and in_channels == out_channels and num_groups > 1:
            in_splits = _split_channels(in_channels, num_groups,uneven_split=True)
            out_splits = _split_channels(out_channels, num_groups, uneven_split=True)
        else:
            # original code
            in_splits = _split_channels(in_channels, num_groups,uneven_split=False)
            out_splits = _split_channels(out_channels, num_groups, uneven_split=False)
...
class MixedAvgPooling(nn.Module):
    def __init__(self, splits, kernel_size, stride):
        super(MixedAvgPooling, self).__init__()
        self.splits = splits
        for idx, k in enumerate(kernel_size):
            padding = _get_padding(k, stride)
            self.add_module(
                str(idx),
                nn.AvgPool2d(kernel_size=k, stride=stride, padding=padding)
            )
    def forward(self, x):
        x_split = torch.split(x, self.splits, 1)
        x_out = [c(x) for x, c in zip(x_split, self._modules.values())]
