...
from pruner import Pruner, Pruner_mixed
...
parser.add_argument('--pruner', default='V1', type=str, help='Choosing pruner, 1) V1, 2) V2, 3) V3')
...
class ForwardSign(torch.autograd.Function):
    ...
    def forward(ctx, x):
        ...
        if args.amp and args.opt_level != 'O1':
            return x_ternary.type(torch.cuda.HalfTensor)
        else:
...
def loss_consistency_without_prefetcher(model, loss_fn, consistency_criterion, input, target, epoch, args):

    input, target = input.cuda(), target.cuda()
    r = np.random.rand(1)
    if args.mixup > 0 and r < args.cutmix_prob and (not args.mixup_off_epoch or epoch < args.mixup_off_epoch):
    ...
    for batch_idx, (input, target) in enumerate(loader):
        ...
        if not args.prefetcher:
            if args.cutmix_prob > 0:
                ...
                loss = loss_consistency_without_prefetcher(model, loss_fn, consistency_criterion, input, target, epoch, args)
            else:
